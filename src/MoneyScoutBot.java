...
public class MoneyScoutBot {
    ...
    static int movesCounterBotMemory = 0;
    ...
    private static String thinkWhatsYourNextMove(int[][] visibleCoins, int[][] visibleWalls,
        ...
        movesCounterBotMemory++;

        if (movesCounterBotMemory == 10) {
            return "s";
        }
        if (movesCounterBotMemory > 10) {
            return "a";
    ...
    private static int[][] getVisibleObjects(int[][] allObjects, int robotPosRow, int robotPosCol) {
        ...
        //Создаём новый массив boolean что бы понять на каких координатах есть объекты
        boolean[] visibleObjectsIndexes = new boolean[allObjects.length];
        //Проверяем в диапазоне квадрата 2*2 от координатов робота есть ли какие-нибудь объекты
        //и если есть то сохраняем true в массив boolean, так мы поймём сколько объектов вокруг робота
        ...
        int visiblesCount = 0;
        //Подсчитываем количество видимых объектов что бы создать новый массив такой длинны
    ...
    private static int[][] getWalls() {
        return new int[][]{{1, 1, 2, 2}, {14, 14, 5, 1}};
    }
